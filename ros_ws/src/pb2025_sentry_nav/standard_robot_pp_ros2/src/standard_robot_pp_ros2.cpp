...
void StandardRobotPpRos2Node::sendData()
{
  ...
  while (rclcpp::ok()) {
    ...
    // =======================================================
    // 修改 3: 在这里添加以下代码以设置固定的测试数据为 1
    // =======================================================
    send_robot_cmd_data_.data.speed_vector.vx = 1.0f;  // 底盘X轴速度设置为1
    send_robot_cmd_data_.data.speed_vector.vy = 1.0f;  // 底盘Y轴速度设置为1
    send_robot_cmd_data_.data.speed_vector.wz = 1.0f;  // 底盘Z轴旋转角速度设置为1

    send_robot_cmd_data_.data.gimbal.pitch = 1.0f;     // 云台Pitch轴角度设置为1
    send_robot_cmd_data_.data.gimbal.yaw = 1.0f;       // 云台Yaw轴角度设置为1

    send_robot_cmd_data_.data.shoot.fric_on = 1;       // 摩擦轮开启 (布尔值, 1代表true)
    send_robot_cmd_data_.data.shoot.fire = 1;          // 开火指令 (1代表开火)

    send_robot_cmd_data_.data.tracking.tracking = 1;   // 视觉追踪目标 (布尔值, 1代表true)
    // =======================================================
